...
void updateRejectedAngles() {
  for (int i = 0; i < rejectedAngleCount; i++) {
    ...
    if (rejectedAngleSweepCount[i] >= 2) {
      ...
      rejectedAngleCount--;
      i--;
      Serial.print(F("Angle un-rejected after 2 sweeps: "));
      Serial.println(rejectedAngles[i]);
    }
  }
}
...
float calculateWeightedScore(HUSKYLENSResult result) {
  ...
  if (isTargetColor && abs(result.width - result.height) <= squareTolerance) {
    // Calculate score: color weight + block size - distance penalty
...
void printResult(HUSKYLENSResult result) {
  if (result.command == COMMAND_RETURN_BLOCK) {
    ...
    if (score > 0) {
      Serial.print(F("Potential Target Block: xCenter="));
...
void loop() {
  if (!huskylens.request()) {
    ...
    while (huskylens.available()) {
      ...
      if (result.command == COMMAND_RETURN_BLOCK) {
        if (!servoLocked && !isAngleRejected(currentCameraAngle)) {
          ...
          Serial.print(F("Servo locked at angle: "));
    ...
    if (foundPotentialTarget) {
      ...
      Serial.print(F("Selected Best Block: xCenter="));
      Serial.print(bestXCenter);
      Serial.print(F(", yCenter="));
      Serial.print(bestYCenter);
      Serial.print(F(", width="));
      Serial.print(bestWidth);
      Serial.print(F(", height="));
