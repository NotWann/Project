...
//Valve - Relay
const int valveOn = 12;
const int valveOff = 13;
...
void loop() 
{
  ...
  if (buttonState == LOW) 
  {
    ...
    else if (currentTime - buttonPressStart >= 1000 && !pumpOn) 
    {
      // Button held for 1 second, turn on pump (commented for testing)
      digitalWrite(pump, HIGH);
      pumpOn = true;
      sound();
      Serial.println("Pump ON (disabled for testing)");
    }
  } 
  else if (buttonHeld) 
  {
    ...
    {
      // Turn off pump on release after long press (commented for testing)
      digitalWrite(pump, LOW);
      pumpOn = false;
      sound();
      Serial.println("Pump OFF (disabled for testing)");
    }
  }
