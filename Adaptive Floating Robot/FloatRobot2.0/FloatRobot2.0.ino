...
void loop() 
{
  ...
  static bool longPressProcessed = false; // Track if long press has been processed
  ...
  if (buttonState == LOW) 
  {
    ...
    if (!buttonHeld) 
    {
      ...
      longPressProcessed = false; // Reset long press flag on new press
    } 
    else if (currentTime - buttonPressStart >= 1000 && !longPressProcessed) 
    {
      //Manually ON and OFF Pump function
      ...
      longPressProcessed = true; // Mark long press as processed
    }
  } 
  /////////Function to manually turn ON and OFF the propeller
  else if (buttonHeld) 
  {
  ...
  if (distance_Bawah <= 8 && distance_Bawah < 999) 
  {
      if (distanceBawahTriggered == false)
      {
        digitalWrite(pump, HIGH);
    ...
    Serial.println("Distance Bawah <= 8cm, Pump OFF, Air Out");
  } 
  //Check water level if above 15cm turn ON pump, Air In ON
  else if (distance_Bawah > 15 && distanceBawahTriggered == false)
  {
    // Turn on pump + Air In valve
    digitalWrite(pump, HIGH);
    setValves(true, true); // Air In ON, Air Out OFF
