...
/*#######################################################*/
int lock_servo = 0;  // Lock servo prevent retrigger

// Servo reset angle time
unsigned long reset_servo_duration = 0;  // Made variable, updated based on set_timer
...
void read_potvalue() {
  ...
  if (flag_mode == 1) {
    ...
    // Set Total Timer:
      ...
      seconds = constrain(map(pot_value, 0, 1000, 0, 59), 0, 59);  // 0-59 seconds
      timer = hours * 3600 + minutes * 60 + seconds;               // Update timer
      save_eeprom(4);
    ...
    // Set Servo ON:
      ...
      reset_servo_duration = (unsigned long)set_timer * 1000UL;                                     // Update reset duration
      save_eeprom(5);
      ...
      servo_rotated_seconds = constrain(map(pot_value, 0, 1000, 0, 59), 0, 59);                     // 0-59 seconds
      set_timer = servo_rotated_hours * 3600 + servo_rotated_minutes * 60 + servo_rotated_seconds;  // Update set_timer
      reset_servo_duration = (unsigned long)set_timer * 1000UL;                                     // Update reset duration
      save_eeprom(7);
...
void timer_countdown() {
  ...
  /******************************************************************/
  // Check for servo reset independently (moved before countdown to handle overlap)
  /******************************************************************/
  if (lock_servo == 1 && currentTime - last_servo_rotated_duration >= reset_servo_duration) {
    lock_servo = 0;
    digitalWrite(buzzer_pin, HIGH);
    buzzerIsOn = true;
    ...
    if (seconds > 0 || minutes > 0 || hours > 0) {
      ...
      if (set_timer > 0 && elapsedTime % set_timer == 0 && elapsedTime > 0 && lock_servo == 0) {
        ...
        if (elapsedTime != lastRotationTime) {  // Prevent multiple triggers in the same second
          ...
          servo.write(100);
          lock_servo = 1;
          last_servo_rotated_duration = currentTime;  // Update rotation time
...
void read_eeprom() {
  ...
  // Update reset duration based on set_timer
  reset_servo_duration = (unsigned long)set_timer * 1000UL;
}
