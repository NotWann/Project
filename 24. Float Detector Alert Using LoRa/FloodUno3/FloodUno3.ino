...
const long sendDelay = 4000; // 4-second delay between sends (longer than Flood A)
const long timeSlotOffset = 2000; // Flood B: 2000ms offset (second 2s of 4s cycle)
  ...
  if (e32ttl.available() > 0) {
    ...
    if (rs.status.code == 1) { // Success
      ...
      if (message[0] == 'S' && strcmp(message, "ST") == 0 && strcmp(message, last_processed_message) != 0) {
        ...
        if (currentMillis % 4000 >= 2000 && currentMillis - lastSendMillis >= sendDelay) { // Flood B time slot
          String response = (lock_status == 1) ? "B1" : "B0";
...
void read_float() {
  ...
  if (current_float_state != last_float_state && currentMillis - lastSendMillis >= sendDelay) {
    if (currentMillis % 4000 >= 2000) { // Flood B sends in second 2 seconds of every 4-second cycle
      if (current_float_state == HIGH && last_sent_state != HIGH) {
        ...
        ResponseStatus rs1 = e32ttl.sendFixedMessage(0, 1, 0x04, "B1");
        Serial.println(rs1.getResponseDescription());
        lock_status = 1;
        ...
        ResponseStatus rs1 = e32ttl.sendFixedMessage(0, 1, 0x04, "B0");
        Serial.println(rs1.getResponseDescription());
        lock_status = 0;
        last_sent_state = LOW;
        lastSendMillis = currentMillis;
...
void setup() {
  ...
  configuration.ADDL = 0x05; // Flood B address
